...
const QMap<QString, QString>& enemySprites() {
    static const QMap<QString, QString> sprites = {
        {"懈怠的短跑者", ":/assets/assets/sprites/gym_sprinter.png"},
        ...
        {"涂鸦书灵", ":/assets/assets/sprites/library_scribble_spirit.png"},
        ...
        {"镜面侍从", ":/assets/assets/sprites/theater_mirror_attendant.png"},
        {"酸液史莱姆", ":/assets/assets/sprites/lab_acid_slime.png"},
        {"爆炸烧杯怪", ":/assets/assets/sprites/lab_beaker.png"},
        {"剧毒试管精", ":/assets/assets/sprites/lab_tube.png"},
        {"塔罗士兵 A", ":/assets/assets/sprites/divination_tarot.png"},
        {"塔罗士兵 B", ":/assets/assets/sprites/divination_tarot.png"},
        ...
        {"纪律巡查使", ":/assets/assets/sprites/office_discipline_inspector.png"},
...
void MainWindow::setupData() {
    ...
    tasks = {
        ...
        {14, "打败最终BOSS逃离轮回", "打通七层地窟最终BOSS，打破诅咒。", "最终BOSS", "伪典校长·零", 1, 0, 0, 1000, 100, ""}
    };
}
...
void MainWindow::setupGamePage() {
    ...
    auto* dungeonLayout = new QHBoxLayout(dungeonPage);
    ...
    auto* dungeonPanel = new QVBoxLayout();
    ...
    dungeonPanel->addWidget(dungeonLabel);
    dungeonPanel->addWidget(dungeonRoomList, 1);
    dungeonPanel->addWidget(demonShopList);
    dungeonPanel->addWidget(exploreBtn);
    dungeonPanel->addWidget(buyDemonBtn);
    dungeonPanel->addWidget(sellDemonBtn);
    dungeonPanel->addWidget(fightRoundBtn);
    dungeonPanel->addWidget(battleMedicineBtn);
    dungeonLayout->addLayout(battlefieldLayout, 3);
    dungeonLayout->addLayout(dungeonPanel, 2);
...
void MainWindow::refreshCharacters() {
    ...
    for (int i = 0; i < party.size(); ++i) {
        ...
        auto* item = new QListWidgetItem(QIcon(professionSprite(party[i].profession)), QString("%1 [%2]\n%3\n技能：\n%4")
...
void MainWindow::refreshCodex() {
    ...
    for (const QString& name : names) {
        ...
        auto* item = new QListWidgetItem(QIcon(enemySprite(e.name)), QString("%1 | 第%2层%3 | HP%4 攻%5 防%6 法抗%7 | 技能：%8")
...
QString MainWindow::professionSprite(const QString& professionName) const {
    return professionSprites().value(professionName);
}

QString MainWindow::enemySprite(const QString& enemyName) const {
    return enemySprites().value(enemyName);
}

QString MainWindow::dungeonScene(int layer) const {
    return dungeonScenes().value(layer, ":/assets/assets/scenes/playground.jpg");
}
...
void MainWindow::refreshBattleScene() {
    ...
    auto addActor = [this](const QString& name, const QString& resource, int hp, int maxHp, const QPointF& center, bool player) {
        const QSize spriteBounds(player ? 195 : 210, player ? 250 : 265);
        ...
        if (!sprite.isNull()) {
            ...
            actor->setPos(center.x() - sprite.width() / 2, center.y() - sprite.height() / 2 - 12);
            if (hp <= 0) actor->setOpacity(0.32);
        }

        constexpr int barWidth = 154;
        constexpr int barHeight = 13;
        const int barX = int(center.x()) - barWidth / 2;
        const int barY = int(center.y()) + 128;
        battleScene->addRect(barX - 2, barY - 21, barWidth + 4, 39, Qt::NoPen, QBrush(QColor(4, 12, 16, 178)));
        ...
        hpFont.setPointSize(8);
        hpText->setFont(hpFont);
        hpText->setPos(barX + barWidth + 7, barY - 3);
    };

    const int frontCount = formationType == 1 ? 1 : 2;
    const QVector<QPointF> frontPlayerPositions = {{270, 292}, {270, 420}};
    const QVector<QPointF> rearPlayerPositions = {{106, 198}, {106, 374}};
    int frontIndex = 0;
    int rearIndex = 0;
    for (int i = 0; i < party.size(); ++i) {
        const bool front = i < frontCount;
        QPointF position = front
            ? frontPlayerPositions.value(frontIndex++, QPointF(270, 292))
            : rearPlayerPositions.value(rearIndex++, QPointF(106, 286));
    ...
    if (inBattle && !battleEnemies.isEmpty()) {
        if (battleEnemies.size() == 1) {
            ...
            addActor(enemy.name, enemySprite(enemy.name), enemy.hp, enemy.maxHp, QPointF(700, 294), false);
        } else {
            const QVector<QPointF> enemyPositions = {{640, 294}, {828, 192}, {828, 385}};
            for (int i = 0; i < battleEnemies.size(); ++i) {
                const auto& enemy = battleEnemies[i];
                addActor(enemy.name, enemySprite(enemy.name), enemy.hp, enemy.maxHp, enemyPositions.value(i, QPointF(820, 294)), false);
            }
        }
    } else {
...
void MainWindow::createInSelectedSlot() {
    ...
    QDialog dialog(this);
    dialog.setWindowTitle("选择职业");
    dialog.setMinimumWidth(780);
    auto* layout = new QVBoxLayout(&dialog);
    auto* title = new QLabel("选择一名角色开始校园轮回", &dialog);
    title->setStyleSheet("font-size:20px;font-weight:700;color:#174d58;");
    layout->addWidget(title);
    auto* grid = new QGridLayout();
    auto* group = new QButtonGroup(&dialog);
    group->setExclusive(true);
    QString prof = names.first();
    for (int i = 0; i < names.size(); ++i) {
        const QString& name = names[i];
        auto* button = new QToolButton(&dialog);
        button->setText(name);
        button->setIcon(QIcon(professionSprite(name)));
        button->setIconSize(QSize(112, 148));
        button->setToolButtonStyle(Qt::ToolButtonTextUnderIcon);
        button->setCheckable(true);
        button->setChecked(i == 0);
        button->setMinimumSize(150, 190);
        button->setStyleSheet("QToolButton{border:1px solid #93aeb1;background:#f9fbfb;padding:6px;} QToolButton:checked{border:3px solid #176b73;background:#e2f1ef;font-weight:700;}");
        group->addButton(button, i);
        grid->addWidget(button, i / 3, i % 3);
    }
    connect(group, QOverload<int>::of(&QButtonGroup::idClicked), &dialog, [&prof, &names](int index) { prof = names[index]; });
    layout->addLayout(grid);
    auto* nameInput = new QLineEdit(&dialog);
    nameInput->setPlaceholderText("输入角色名称");
    auto* form = new QFormLayout();
    form->addRow("角色名称", nameInput);
    layout->addLayout(form);
    auto* buttons = new QDialogButtonBox(QDialogButtonBox::Cancel | QDialogButtonBox::Ok, &dialog);
    buttons->button(QDialogButtonBox::Ok)->setText("开始冒险");
    connect(buttons, &QDialogButtonBox::accepted, &dialog, [&dialog, nameInput] { if (!nameInput->text().trimmed().isEmpty()) dialog.accept(); });
    connect(buttons, &QDialogButtonBox::rejected, &dialog, &QDialog::reject);
    layout->addWidget(buttons);
    if (dialog.exec() != QDialog::Accepted) return;
    QString roleName = nameInput->text().trimmed();
...
QVector<MainWindow::EnemyData> MainWindow::makeEnemyGroup(int layer, bool elite, bool boss) const {
    if (boss) {
        return {{"伪典校长·零", "首领", 7, 900, 900, 72, 45, 38, 36, 600, 120, false, true, true, 0, 0, 0, "伪典校规：攻击全体并优先压低后排"}};
    }
    int hp = 70 + layer * 28;
    int atk = 12 + layer * 7;
    int def = 4 + layer * 3;
    const QStringList regularNames = {
        "懈怠的短跑者|驼背的铅球手|体操幽灵",
        "目录魔像|涂鸦书灵|禁书管理员",
        "微笑小丑|镜面侍从|悲情女主角",
        "酸液史莱姆|爆炸烧杯怪|剧毒试管精",
        "塔罗士兵 A|塔罗士兵 B|时针幽灵",
        "教导处铁腕·左|教导处铁腕·右|纪律巡查使"
    };
    const QStringList names = regularNames.value(std::clamp(layer, 1, 6) - 1).split('|');
    QVector<EnemyData> group = {
        {names[0], "小怪", layer, hp, hp, atk, 0, def, def, 30 + layer * 12, 6 + layer * 3, false, false, false, 0, 0, 0, "层级技能：普通攻击"},
        {names[1], "小怪", layer, hp + 20, hp + 20, atk + 4, 0, def + 2, def, 35 + layer * 12, 7 + layer * 3, false, false, true, 0, 0, 0, "层级技能：越过前排攻击后排"},
        {names[2], "小怪", layer, hp - 10, hp - 10, atk, atk + 8, def, def + 3, 32 + layer * 12, 8 + layer * 3, false, false, false, 0, 0, 0, "层级技能：法术伤害"}
    };
    if (elite) {
        const QStringList eliteNames = {"折翼的百米王者", "扉页守护者", "无面舞者", "畸变融合体", "时计塔的囚徒", "铁锈执念·陈暮"};
        group[0].name = eliteNames.value(std::clamp(layer, 1, 6) - 1);
        group[0].kind = "精英";
        group[0].maxHp += 120;
